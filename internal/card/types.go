...
// UpdateParser extends the parser with new card types
func (p *Parser) parseRecord(record []string, colIndex map[string]int) (Card, error) {
    cardType := CardType(record[colIndex["Type"]])
    ...
    case TypeCreature:
        ...
        return &Creature{
            ...
            Attack:   attack,
            Defense:  defense,
            SubType:  record[colIndex["SubType"]],
        }, nil
    case TypeArtifact:
        return &Artifact{
            BaseCard:    baseCard,
...
// Helper functions for parsing specific card attributes
